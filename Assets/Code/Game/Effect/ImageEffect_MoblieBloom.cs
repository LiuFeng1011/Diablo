...
public class ImageEffect_MoblieBloom : MonoBehaviour {
    ...
    void Update(){
        ...
        {
            threshold = threshold + (targetThreshold - threshold) * 0.1f;
        }
    }

    public void Open(float rate){
        threshold = 1.0f;
        targetThreshold = rate;
    }
	...
	{	
		...
		if(threshold != 0 && intensity != 0){
	        ...
	        BloomMaterial.SetVector ("_Parameter", new Vector4(BlurSize*1.5f, 0.0f, intensity,1-threshold));	
